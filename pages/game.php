...
<div id="container" >
    ...
    <div id="blockDisplay" style="width: <?=($contWidth + 2 * 100)?>px; height: <?=($contHeight + 2 * 100)?>px;"></div>
    <div id="statusBar" style="height: 10px;">
        <div class="info"></div>
        <div class="showhide"><div class="arrows show">
            <p class="arrow"></p><p class="arrow"></p>
        </div></div>
    </div>
    <div id="controlBar" style="top: <?=$curUser['user_setScreen'] - 50?>px; display: block;">
        <div class="finishStep"></div>
    </div>
    <div id="game" style="width: <?=($contWidth + 2 * 100)?>px; height: <?=($contHeight + 2 * 100)?>px; margin-top: 0px; margin-left: 0px;">
        <? for($x = 0; $x <= $sizes['horizontal'] + 1; $x++): ?>
            <? for($y = 0; $y <= $sizes['vertical'] + 1; $y++): ?>
                <? if($x == 0 || $y == 0 || $x == $sizes['horizontal'] + 1 || $y == $sizes['vertical'] + 1): ?>
                <div class="card sea" data-posX="<?=$x?>" data-posY="<?=$y?>" style="left: <?=($x * 120)?>px; top: <?=($y * 120)?>px;"></div>
                <? endif; ?>
            <? endfor; ?>
        <? endfor; ?>
