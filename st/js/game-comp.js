...
DGame['Map'] = can.Control.extend({
    ...
    init : function(){
        ...
        setTimeout(function(){
            $('#game')
                .on('click', '.sea', function(e){ return $.proxy(controller.seaClick, controller)($(this), e); })
                .on('click', '.pirateUnit', function(e){ return $.proxy(controller.pirateClick, controller)($(this), e); })
                .on('mouseenter', '.pirateUnit', function(e){ return $.proxy(controller.pirateMouseEnter, controller)($(this), e); })
                .on('mouseleave', '.pirateUnit', function(e){ return $.proxy(controller.pirateMouseLeave, controller)($(this), e); })
                .on('click', '.sea .ship', function(e){ return $.proxy(controller.shipClick, controller)($(this), e); })
                .on('mouseenter', '.sea .ship', function(e){ return $.proxy(controller.shipMouseEnter, controller)($(this), e); })
                .on('mouseleave', '.sea .ship', function(e){ return $.proxy(controller.shipMouseLeave, controller)($(this), e); });
            $('#statusBar')
                .on('click', '.unchange', function(e){ return $.proxy(controller.unchange, controller)($(this), e); });
        }, 500);
    ...
    unitMove: function(from, who, where){
        ...
        setTimeout(function(){ DGame.Helpers.circleMenuReset(); }, 1000);
        opt.cUnit.show(who);
        ...
        if(where.hasClass('sea')) {
            setTimeout(function(){ opt.cUnit.hide(who); }, 1000);
        } else {
        ...
        if(field.hasClass('sea')) {
            ...
            if(fieldInfo.field_type == 'TRANSPORTER' && fieldInfo.field_direction == '1') {
                ...
                if(goto.length && (goto.hasClass('mapField') || (goto.hasClass('sea') && goto.find('.ship.my').length))) {
                    setTimeout(function(){ controller.unitMove(field, who, goto); }, 1000);
                }
            }
        }
    },
        ...
        switch(fieldInfo.field_id) {
            ...
            case '8'://пушка
                ...
                if(fieldInfo.direction == 4) {
                    fly = 'flyUp';
                    revert = true;
                    units = $('.pirateUnit'+ifMy+'[data-posx='+fieldPos.x+']').filter(function(){ return $(this).attri('data-posy') < fieldPos.y; });
                }
                if(fieldInfo.direction == 3) {
                    fly = 'flyLeft';
                    revert = true;
                    units = $('.pirateUnit'+ifMy+'[data-posy='+fieldPos.y+']').filter(function(){ return $(this).attri('data-posx') < fieldPos.x; });
                }
            ...
            case '18':// подзорная труба
                ...
                if(fieldInfo.direction == 4) {
                    revert = true;
                    fields = $('.mapField[data-posx='+fieldPos.x+']').filter(function(){ return $(this).attri('data-posy') < fieldPos.y; });
                }
                if(fieldInfo.direction == 3) {
                    revert = true;
                    fields = $('.mapField[data-posy='+fieldPos.y+']').filter(function(){ return $(this).attri('data-posx') < fieldPos.x; });
                }
                if(fieldInfo.direction == 2) {
                    fields = $('.mapField[data-posx='+fieldPos.x+']').filter(function(){ return $(this).attri('data-posy') > fieldPos.y; });
                }
